...
def apgd_train(model, x, y, norm, eps, n_iter=10, use_rs=False, loss='ce',
    ...
    for i in range(n_iter):
        ...
        prob = torch.tensor([prob_logits[i][y[i]].detach() for i in range(len(y))])
    ...
    return x_best, acc, prob, loss_best, x_best_adv

def train_clean(args, epoch, model, train_loader, optimizer, lr_schedule):
    
    lr = 0
    # num_data = 0
    train_natural_loss = 0
    running_acc_ep_nat = 0.
    num_batches = 0
    ...
    with tqdm(train_loader, unit="batch") as tepoch:
        for i, (data, target) in enumerate(tepoch):
            ...
            train_natural_loss += loss.item()
            running_acc_ep_nat += (logit.max(dim=-1)[1] == target).cpu().float().sum().item()
            
            tepoch.set_postfix({'loss': train_natural_loss / (i + 1), 'acc': running_acc_ep_nat / (i + 1) / args.batch_size})
    ...
    train_natural_loss = train_natural_loss / len(train_loader)

    return model, train_natural_loss

def apgd_train_kl(model, x, x_adv_s, y, norm, eps, n_iter=10, use_rs=False, loss='ce',
    verbose=False, is_train=True):
    assert not model.training
    device = x.device
    ndims = len(x.shape) - 1
    ...
    for i in range(n_iter):
        ...
        #with torch.enable_grad()
        logits = model(x_adv)
        logits_old = model(x_adv_s)

        loss_indiv = criterion_indiv(logits, y)
        # loss_indiv = criterion_indiv(logits_old, y)

        # kl loss
        loss_kl = 0

        criterion_kl = nn.KLDivLoss(reduction='mean').cuda()
        old_correct_indices =  (logits_old.max(dim=-1)[1] == y).detach()
        # source_sel_kl, target_sel_kl = logits_old[old_correct_indices], logits[old_correct_indices]
        source_sel_kl, target_sel_kl = logits_old, logits
        if len(old_correct_indices) > 0:
            loss_kl = criterion_kl(F.log_softmax(target_sel_kl+1e-12, dim=1), F.softmax(source_sel_kl, dim=1)) * (old_correct_indices.sum() / len(y))

        # loss = loss_indiv[~old_correct_indices].sum() * (1 - old_correct_indices.sum() / len(y)) + 5 * loss_kl
        loss = loss_indiv.sum() + 5 * loss_kl
        # loss = loss_kl
