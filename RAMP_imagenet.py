...
from utils import get_params_no_decay
import utils
        ...
        if True:
            ...
            with tqdm(train_loader, unit="batch") as tepoch:
                ...
                for i, (x_loader, y_loader) in enumerate(tepoch):
                    ...
                    # update lr
                    lr = lr_schedule(i+1, len(train_loader))
                    # print(lr)
                    ...
                    x_tr_s, _, _, loss_best_s, _ = apgd_train(model, x, y, norm=args.l_norms[cur_norm_source],
                                eps=args.l_eps[cur_norm_source], n_iter=5, is_train=True)
                                     
                    # l1
                    x_tr_t, _, _, loss_best_t, _ = apgd_train(model, x, y, norm=args.l_norms[cur_norm_target],
                                eps=args.l_eps[cur_norm_target], n_iter=15, is_train=True)
